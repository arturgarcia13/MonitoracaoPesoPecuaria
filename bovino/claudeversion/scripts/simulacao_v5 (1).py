...
TETO BIOLÓGICO
──────────────────────────────────────────────────────────────────
  PESO_TETO = 289,30 + 3 × sqrt(1354,58) = 400,0 kg
  Fonte: de Souza 2018 — média MQ machos Nelore P365 + 3σ_p
  Status: 🟡 ÁLGEBRA sobre dois literais (289,30 kg e σ²_p=1354,58)
  Justificativa: cobre 99,87% da distribuição fenotípica Nelore P365;
  aplicado globalmente como limite conservador para todas as raças.
=============================================================================
"""
...
# ── Teto biológico global ────────────────────────────────────────────────────
# Fonte: de Souza et al. (2018) — média MQ machos Nelore P365 + 3σ_p
# = 289,30 + 3 × sqrt(1354,58) = 289,30 + 3 × 36,80 = 400,0 kg
# Cobre 99,87% da distribuição fenotípica Nelore P365.
# Aplicado globalmente: conservador para todas as raças, elimina apenas
# combinações extremas (GMD outlier + PN alto + ε grande simultâneos).
PESO_TETO = 289.30 + 3.0 * np.sqrt(SIGMA2_P_365)   # = 400.0 kg
...
def gerar_dados(n: int, seed: int = SEED) -> pd.DataFrame:
    ...
    # Teto superior: média_M_Nelore_P365 + 3σ_p = 400 kg (de Souza 2018)
    # Piso inferior: 85% do PN individual
    Peso  = np.clip(Peso, PN * 0.85, PESO_TETO)
